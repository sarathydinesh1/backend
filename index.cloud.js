...
app.post('/customer/upload', upload.single('image'), authenticateToken, async (req, res) => {
  ...
  if (!mobile || !req.file) return res.status(400).json({ message: 'Mobile and image required.' });
  try {
    const fileName = `customer_${mobile}_${Date.now()}.jpg`;
    const file = bucket.file(fileName);
    await file.save(req.file.buffer, {
      metadata: { contentType: req.file.mimetype },
      public: true,
...
// Tailor profile image upload to Firebase
app.post('/tailor/upload', upload.single('image'), authenticateToken, async (req, res) => {
  const { mobile } = req.body;
  if (!mobile || !req.file) return res.status(400).json({ message: 'Mobile and image required.' });
  try {
    const fileName = `tailor_${mobile}_${Date.now()}.jpg`;
    const file = bucket.file(fileName);
    await file.save(req.file.buffer, {
      metadata: { contentType: req.file.mimetype },
      public: true,
